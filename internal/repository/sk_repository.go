...
func (r *SKRepository) GetSKDetail(ctx context.Context, skNo string) ([]SKBedRow, error) {
	...
	for rows.Next() {
		...
		if clinicID.Valid {
			row.ClinicID = clinicID.String
		}
		if classRoomID.Valid {
			row.ClassRoomID = classRoomID.String
		}
		if kelas.Valid {
			row.Kelas = kelas.String
		}
		if bed.Valid {
			row.Bed = int(bed.Int64)
		}
		if idTTSiranap.Valid {
			row.IDTTSiranap = idTTSiranap.String
		}
		if ruangSiranap.Valid {
			row.RuangSiranap = ruangSiranap.String
		}
		if kelasSiranap.Valid {
			row.KelasSiranap = kelasSiranap.String
		}
		if covid.Valid {
			row.Covid = int(covid.Int64)
		}
		if siranap.Valid {
			row.Siranap = siranap.String
		}
		if jmlRuang.Valid {
			row.JmlRuangSiranap = int(jmlRuang.Int64)
		}
		if kodekelas.Valid {
			row.KodeKelas = kodekelas.String
		}
		if namakelas.Valid {
			row.NamaKelas = namakelas.String
		}
		if namaruang.Valid {
			row.NamaRuang = namaruang.String
		}
		if kris.Valid {
			row.Kris = kris.String
		}
		if kamar.Valid {
			row.Kamar = kamar.String
		}
