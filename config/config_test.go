...
func TestLoad_EnvironmentVariables(t *testing.T) {
	...
	if cfg.Database.Host != "test-host" {
		t.Errorf("expected DBHost to be 'test-host', got %q", cfg.Database.Host)
	}

	if cfg.Operational.AppPort != 8888 {
		t.Errorf("expected AppPort to be 8888, got %d", cfg.Operational.AppPort)
	}
	...
	if cfg.Security.MaxBodyBytes != 2048 {
		t.Errorf("expected MaxBodyBytes to be 2048, got %d", cfg.Security.MaxBodyBytes)
	}
}
